...
namespace GamerUncle.Pipeline.Tests
{
    ...
    public class PipelineConfigurationTests
    {
        ...
        public void Pipeline_ShouldHaveFunctionAppHealthCheck()
        {
            ...
            // Act & Assert
            Assert.IsTrue(pipelineContent.Contains("Verifying Function App base URL accessibility"), 
                "Pipeline should verify function app base URL before triggering functions");
            Assert.IsTrue(pipelineContent.Contains("MAX_URL_RETRIES=5"), 
                "Pipeline should have retry logic for function app health checks");
        ...
        public void Pipeline_ProductionStagesShouldBeGatedByDevTests()
        {
            ...
            // Act & Assert
            // Production stages should depend on FunctionalTestsDev succeeding
            Assert.IsTrue(pipelineContent.Contains("- FunctionalTestsDev"), 
                "Production deployment stages should depend on FunctionalTestsDev");
            Assert.IsTrue(pipelineContent.Contains("dependencies.FunctionalTestsDev.result, 'Succeeded'"), 
                "Production deployment stages should require FunctionalTestsDev to succeed");
