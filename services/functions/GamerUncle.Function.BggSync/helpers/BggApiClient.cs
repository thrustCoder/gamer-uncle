...
namespace GamerUncle.Functions.Helpers
{
    public class BggApiClient
    {
        ...
        public BggApiClient(HttpClient httpClient, string? bearerToken = null)
        {
        ...
        public async Task<GameDocument?> FetchGameDataAsync(string gameId)
        {
            try
            {
                var response = await _httpClient.GetAsync($"https://boardgamegeek.com/xmlapi2/thing?id={gameId}&stats=1");
